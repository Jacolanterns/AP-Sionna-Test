...
def process_data(data, floor_filters):
    ...
    # Convert _value column to numeric, handling any non-numeric values
    print("Converting _value column to numeric...")
    data['_value'] = pd.to_numeric(data['_value'], errors='coerce')
    # Remove rows where _value couldn't be converted to numeric
    data = data.dropna(subset=['_value'])
    print(f"Records after numeric conversion: {len(data)}")
