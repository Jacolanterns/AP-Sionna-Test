...
/usr/bin/python3 fetch_ap_data.py --start_datetime "$start_datetime" --end_datetime "$end_datetime" --interval "$interval" --bucket "$bucket" --measurement "$measurement" --fields $fields --building "$building" --output "$output_fetch"
...
/usr/bin/python3 data_transformations.py -i "$output_fetch" -f "$floors_folder" -o "$output_transform"
...
/usr/bin/python3 model_training_and_testing.py --input_file "$output_transform" \
    --mode "$mode" \
